...
// adding id for image:
const RoboHashImage = document.getElementById('RoboHashRandomImages')
...
function updateBatteryStatus(battery) {
    ...
    const diffbatteryPercnt = battery.level *100;
    chargeLevel.textContent = (battery.level * 100) + "%";
    chargeMeter.value = (battery.level * 100);

    const imageURL = `https://robohash.org/${diffbatteryPercnt}?set=set4`
